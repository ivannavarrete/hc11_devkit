...
#include "cmd.h"			// XXX still fucked up
#include "disasm.h"
#include "sfile.h"
#include "ui_txt.h"
...
// XXX PLAYGROUND BEGIN XXX //
	//UI_txt ui;

	//ui.ShowMsg("die sucker..\n");

	//struct mcu_env env;
	//ui.GetEnvOpt(&env);
// XXX PLAYGROUND END XXX //

	try {
		ui = new UI_txt();
		Serial serial = Serial("/dev/ttyS1", 1200);
		Debugger debugger = Debugger(&serial, &env);
...
//			delete cmd;
	...
	// handle all major exceptions here
	} catch (Error &err) {
		cout << "fatal exception in " << err.method << ": " << err.error <<endl;
	} catch (...) {
		cout << "CAUGHT UNHANDLED EXCEPTION! GAME OVER, MAN! GAME OVER!!!!\n";
	}
}
